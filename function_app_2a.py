...
def get_document_hashes_from_storage(storage_account="stbtpuksprodcrawler01", container="document-hashes"):
    """Retrieve stored document hashes from Azure Storage for change detection"""
    try:
        access_token = get_managed_identity_token()
        if not access_token:
            return {}
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            logging.info(f'Retrieved {len(hash_data)} stored document hashes')
            return hash_data
            
    except urllib.error.HTTPError as e:
        if e.code == 404:
            logging.info('No previous hash file found - first run')
            return {}
        else:
            logging.error(f'Error retrieving hashes: {e.code} {e.reason}')
            return {}
    except Exception as e:
        logging.error(f'Error retrieving document hashes: {str(e)}')
        return {}

def store_document_hashes_to_storage(hash_data, storage_account="stbtpuksprodcrawler01", container="document-hashes"):
    """Store document hashes to Azure Storage for change detection"""
    try:
        access_token = get_managed_identity_token()
        if not access_token:
            return False
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            if response.status == 201:
                logging.info(f'Successfully stored {len(hash_data)} document hashes')
                return True
            else:
                logging.error(f'Failed to store hashes: {response.status}')
                return False
