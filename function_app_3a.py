...
def scheduled_crawler(mytimer: func.TimerRequest) -> None:
    """Step 3a: Timer trigger function that crawls websites every 4 hours with full change detection"""
    logging.info('Step 3a: Scheduled crawler started - 4-hour automated crawling with change detection!')
    
    # Get configured websites
    configured_sites = [
        "https://www.legislation.gov.uk/uksi/2024/1052/contents",  # Known working site
    ...
    for site_url in configured_sites:
        try:
            logging.info(f'Step 3a: Processing site - {site_url}')
            ...
            logging.info(f'Step 3a: Found {len(result["documents"])} documents on {site_url}')
            ...
            for i, doc in enumerate(result["documents"]):
                try:
                    logging.info(f'Step 3a: Processing document {i+1}/{len(result["documents"])} - {doc["filename"]}')
                    ...
                    if download_result["success"]:
                        ...
                        if should_upload:
                            ...
                            if storage_result["success"]:
                                total_uploaded += 1
                                logging.info(f'Step 3a: Uploaded {doc["filename"]} - Status: {status}')
                            else:
                                logging.error(f'Step 3a: Upload failed for {doc["filename"]} - {storage_result.get("error", "Unknown error")}')
                        else:
                            logging.info(f'Step 3a: Skipped upload for {doc["filename"]} - Status: {status}')
                            
                    else:
                        logging.error(f'Step 3a: Download failed for {doc["filename"]} - {download_result["error"]}')
                        
                except Exception as doc_error:
                    logging.error(f'Step 3a: Error processing document {doc["filename"]}: {str(doc_error)}')
            
            # Store updated hashes for next run
            if current_hashes:
                store_success = store_document_hashes_to_storage(current_hashes)
                if store_success:
                    logging.info(f'Step 3a: Successfully stored {len(current_hashes)} document hashes')
                else:
                    logging.error('Step 3a: Failed to store document hashes')
                    
        except Exception as site_error:
            logging.error(f'Step 3a: Error processing site {site_url}: {str(site_error)}')
    
    # Log summary
    logging.info(f'Step 3a: Scheduled crawl complete - Processed: {total_processed}, New: {total_new}, Changed: {total_changed}, Unchanged: {total_unchanged}, Uploaded: {total_uploaded}')
