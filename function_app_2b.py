...
def search_site(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Step 2b: Download and upload ALL documents found
        download_results = []
        successful_downloads = 0
        successful_uploads = 0
        
        if result["documents"]:
            logging.info(f'Step 2b: Processing {len(result["documents"])} documents')
            
            for i, doc in enumerate(result["documents"]):
                try:
                    logging.info(f'Step 2b: Processing document {i+1}/{len(result["documents"])} - {doc["filename"]}')
                    download_result = download_document(doc["url"])
        ...
        return func.HttpResponse(
            json.dumps({
                ...
                "message": "Step 2b: Multi-document processing with download and Azure Storage upload"
            }),
            status_code=200,
            mimetype="application/json"
        )
