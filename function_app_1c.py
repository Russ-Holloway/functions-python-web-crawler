...
def search_site(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Step 1c: Download first document and upload to Azure Storage
        download_info = None
        if result["documents"]:
            first_doc = result["documents"][0]
            try:
                logging.info(f'Step 1c: Downloading first document - {first_doc["filename"]}')
                download_result = download_document(first_doc["url"])
                if download_result["success"]:
                    # Step 1c: Upload to Azure Storage
                    storage_result = upload_to_blob_storage_real(download_result["content"], first_doc["filename"])
        ...
        return func.HttpResponse(
            json.dumps({
                ...
                "message": "Step 1c: Enhanced document search with download and Azure Storage upload"
            }),
            status_code=200,
            mimetype="application/json"
        )
